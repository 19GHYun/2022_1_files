...
이젠 read write 대신 fgets(fscanf)랑 fprintf, perror을 쓴다.

그리고 fifo파일 직접 만들거임 // 직접 만드는 함수는 mkfifo(c_to_s, 0600) mkfifo(s_to_c, 0600) ..
                          //connect_to_clinet 함수 맨 앞에 삽입.

근데 보면 fgets(fscanf)랑 fprintf에 보면 각각 핸들값이 0이랑 1(2)임.

근데 우리가 핸들값0과 1을 각각 큐를 지정하고 잇게 바꿨자너. 그래서 FIFO파일로 통신이 되는거임.

아무튼 이렇게 쉽게 서버로 전환할수 있다.....

그리고 실습노트 참고바람

---------
cmdc를 fork사용하여 2개를 만들어서 하나는 키보드 -> 

12장

서버가 두 번 이상 메시지를 보내면 (printf()), cmdc는 몇 번에 걸쳐 받아야할까? 

12장 7페이지 참고

문제는 언제,몇개를 보낼지 모름. 

이 문제 해결방법. fork로 cmd복제, 또는 쓰레드 사용.

fork로 복제하면, 부모 cmd는 키보드에서 대기만 하다가 데이터 오면 서버로 보내주고, 자식 cmd는 서버에서 오는거 대기하다가 데이터 오면 모니터로 보냄.

강의노트에없는거

사용 안하는 프로그램 close

recv_output_loop -> 프로그램 꺼지는거 같은게 없으면 무한반복함

근데 이제 또 문제가 있다. srv3에서 컨트롤 c 해서 끊거나 또는 cmdc2에서 exit입력하면 부모 cmdc2가 정상종료 안함.

cmdc2에서 컨트롤c하면 정상종료

이걸 해결해보자..

부모 자식cmdc3 이 죽으면 서버의 fgets에서 null point가 리턴이 되서 서버는 뒤짐.

서버가 뒤지면 자식 cmdc3에 read가 0이 리턴되면서 뒤짐. 그리고 sigchld_signal이 호출되서 부모에게 전달이 되고,
...
-> 연결을 끊고 입력을 0으로 받음. -> 여기서 0은 커널 내의 FIFO 메시지 큐임. 원래 입력 연결은 키보드였음.

연결을 끊고 출력을 1로 함..  -> 여기서 1은 커널 내의 FIFO 메시지 큐. 원래 출력 연결은 모니터였음.

통신채널만 dup2으로 바꿔주면 운영체제 내부가 알아서 다 잡아줘서 찾아간다.(13장 7페이지)

cmd에서 fgets 랑 printf 쓸때 나오는 함수는 각각 read(0,...) write(1,...) 이런식임.

그때 0이랑 1은 우리가 바꿨다. 그래서 메시지 큐에 가는거임

cmd로 execl해도 이렇게 잘 된다. 운영체제에 그대로 정보가 남아있어서 프로그램이 바뀌어도 0번과 1번으로 쓰면 우리가 dup2 했던 거 그대로 통신이 이루어진다.

execl은 나를 죽이고 다른 프로그램으로 대체하는 것

execl("../cmd/cmd", "cmd", NULL);

-> 첫번째 칸은 어떤 위치의 파일인지, 두번째 칸은 main함수에 보면 int argc, char *argv[] 인데 이 두개의 인자를 cmd파일의 cmd로 받는다는 것인데

cmd파일의 cmd의 포인터는 argc랑 argv[0]임. 그래서 이 두개의 값이 main(int argc, char *argv[])에 넘어감.

execl -> l은 list. execvp에서 v는 vector 배열 p는 명령어 이름 주면 한번에 찾아가주는 그런것...

char argv[0]의 포인터 따라가면 그 프로그램의 이름이 나옴 이것은 약속

그래서 두번째 칸이 cmd인 이유가 있다. 왠만해서 프로그램 이름 넘겨주면 됨.

argc는 최소값이 1임. 해보면안다.

프로그램만 쏙 바뀐거라 13장 7페이지에 있는 기록은 그대로 남아있다 

fork 해서 0이면 자식 0보다 크면 부모

다 close 하면 0부터 순서대로 채워짐

cat f1 | grep "abc" | wc &

-> f1의 출력이 abc의 입력 , abc의 출력이 wc의 입력이 됨.

해석하면 f1의 파일에 abc라는 단어가 있는 걸 찾아서 그 갯수를 나오게 한다.

백그라운드로 도는건 키보드로 받으면 안됨. 받게되면 영원히 스톱이 된다
